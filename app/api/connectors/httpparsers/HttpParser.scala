...
trait HttpParser extends Logging {
  ...
  private val hipErrorReads: Reads[DownstreamErrorCode] = (__ \ "errors").read[DownstreamErrorCode]
  ...
  def parseErrors(response: HttpResponse): DownstreamError = {
    ...
    lazy val hipError       = response.validateJson(hipErrorReads).map(err => DownstreamErrors(List(err)))
    ...
    singleError orElse multipleErrors orElse hipError orElse bvrErrors getOrElse unableToParseJsonError
  }
